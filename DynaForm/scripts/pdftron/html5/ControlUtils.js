    ...
    function initializeReaderControl() {
      ...
      if (queryParams.getBoolean('startOffline')) {
        ...
        window.ControlUtils.byteRangeCheck(function(status) {
          ...
          window.readerControl.loadDocument(doc, { streaming: streaming, workerTransportPromise: workerTransportPromise, pdftronServer: pdftronServer });
        }, function() {
          // some browsers that don't support the range header will return an error
          window.readerControl.loadDocument(doc, { streaming: true, workerTransportPromise: workerTransportPromise, pdftronServer: pdftronServer });
        });
      }
    }
