<template>
	...
	>
		<b-alert
			show
			:variant="variant"
			:dismissible="DisplayValues.dismissible"
		>
			<a href="#"
...
export default {
  ...
  props: [
	...
	data () {
		return {
			...
			DisplayValues: {
				...
				type: this.type == null ? "text" : this.type, //text | link
				template: this.template,
				dismissible: this.dismissible == null ? false : this.dismissible,
			}
	...
	created: function() {
		...
		this.onFilterEvent("_Render", 263, this.guid + this.formType + this.name, (self, watchedField, clearValue = false) => {
